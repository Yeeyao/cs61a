...
def accumulate(combiner, base, n, term):
    ...
    n = n - 1
    while n > 0:
        res = combiner(term(n), res)
        n = n - 1
    return combiner(base, res)
...
def make_repeater(f, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return compose1(accumulate(f, 1, n, f), accumulate(f, 1, n, f))
    # return accumulate(f, 1, n, compose1(f, f))
    def make_repeater2(m): 
        res = f(m)
        counter = n - 1
        while counter > 0:
            res = f(res)
...
def count_change(amount):
    ...
    if part > amount:
        part = part // 2
    return count_partitions(amount, part)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda n: 1 if n == 1 else mul(1, 1 if sub(n, 1) == 0 else mul(n, sub(n, 1)))